...
- Fixed a first-run experience gap: a fresh `docker compose up --build` now auto-seeds the `demo` project with realistic sample traffic (REST, GraphQL, EVM RPC, an already-detected incident) the first time it boots against a project with zero events, so evaluators see a populated dashboard immediately without installing Node.js locally or running the seed script by hand. The seeding logic reuses the same event generator as `npm run seed:demo` (now extracted into `@sentinel/shared`) and goes through the real ingestion queue, so seeded data is processed identically to live traffic. It never re-seeds once a project has any events, and never touches a real signed-up user's project. Documented in a new README "Quick Start" section.
